...
def deconvolute_core_mse(bulk,sc_ref):

    def calculate_square_error(a,sc_ref,bulk):
        ...
        expected =  expected_counts*total_counts
        square_error = np.mean((bulk - expected)**2)
        return square_error

    def optimize_pro_mse(bulk_sample, sc_ref, a_init):
        # 超参数设置
        num_iterations = 50
        ...
        for iteration in range(num_iterations):
            result = minimize(
                fun=lambda a: calculate_square_error(a, sc_ref, bulk_sample),
                x0=a_current,
                method='L-BFGS-B',
                bounds=bounds
            )
    ...
    for i in range(num_sample):
        ...
        cell_pro.append(optimize_pro_mse(bulk.iloc[:,i], sc_ref, a_init))
